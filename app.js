...
async function loadState() {
  try {
    ...
    if (data && data.app_state) {
      ...
      console.log('Dossiers chargés :', appState.dossiers);
  ...
  appState.receipts = [
    {
      ...
      chancelleriePercent: 100 - appState.settings.judicialTreasuryPercentage,
      treasuryAmount: Math.round((6000 * appState.settings.judicialTreasuryPercentage) / 100),
      chancellerieAmount: 6000 - Math.round((6000 * appState.settings.judicialTreasuryPercentage) / 100),
...
function renderReceiptTable() {
  ...
  elements.receiptList.innerHTML = validReceipts.map((receipt) => {
    ...
    const chancelleriePercent = receipt.chancelleryPercent ?? (100 - treasuryPercent);
    const allocation = receipt.allocation || `${treasuryPercent}% Trésor / ${chancelleriePercent}% Chancellerie`;
    const treasuryAmount = Number(receipt.treasuryAmount ?? Math.round((receipt.amount || 0) * treasuryPercent / 100));
    const chancellerieAmount = Number(receipt.chancelleryAmount ?? ((receipt.amount || 0) - treasuryAmount));
      ...
      <tr>
        ...
        <td>
          <div class="transfer-cell">
            ...
            <div class="transfer-amount ${chancelleryTransferred ? 'status-green' : 'status-red'}">${formatMoney(chancellerieAmount)}</div>
...
function showReceiptModal() {
  ...
  const computeReceiptSummary = () => {
    ...
    const chancelleriePercent = 100 - treasuryPercent;
    const treasuryAmount = Math.round((amount * treasuryPercent) / 100);
    const chancellerieAmount = amount - treasuryAmount;
    treasuryPercentEl.textContent = `${treasuryPercent}%`;
    chancelleriePercentEl.textContent = `${chancelleriePercent}%`;
    treasuryAmountEl.textContent = formatMoney(treasuryAmount);
    chancellerieAmountEl.textContent = formatMoney(chancellerieAmount);
    return { treasuryPercent, chancelleriePercent, treasuryAmount, chancellerieAmount };
  };
  ...
  form.addEventListener('submit', (event) => {
    ...
    const receipt = {
      ...
      type,
      amount,
      allocation: `${summary.treasuryPercent}% Trésor / ${summary.chancelleriePercent}% Chancellerie`,
      treasuryPercent: summary.treasuryPercent,
      chancelleriePercent: summary.chancelleriePercent,
      treasuryAmount: summary.treasuryAmount,
      chancellerieAmount: summary.chancellerieAmount,
    ...
    saveState();
    logAction(appState.currentUser.name, 'Enregistrement d’une recette', type, reference, `Recette de ${formatMoney(amount)} enregistrée (${summary.treasuryPercent}% Trésor / ${summary.chancelleriePercent}% Chancellerie).`);
    renderReceiptTable();
    renderDashboard();
    ...
    <div class="modal-card">
      ...
      <form id="receipt-edit-form">
        ...
        <div class="receipt-summary">
          ...
          <div class="summary-line"><span>Pourcentage Chancellerie</span><strong id="receipt-edit-chancellery-percent">${receipt.chancelleriePercent}%</strong></div>
